...
// Time Complexity: O(n) running a linear loop to calculate the min cost
...
public class SolutionTabulation {
    public int minCostClimbingStairs(int[] cost) {
        ...
        // need to reach at the top floor, i.e. reach out of the stairs
        // we need to climb one more step than the total stairs
        int size = cost.length + 1;
        // dp array to store min costs for every steps
        int[] minCosts = new int[size];
        
        // base cases
        minCosts[0] = 0; // we can start from index 0 with no cost
        minCosts[1] = 0; // we can also start from index 1 with no cost

        // we have the min cost to climb first to indices
        // we will start from index 2 until reaching the top
        for (int i=2; i<size; i++) {
            // cost to climb current stair using the stair which is 1 step bellow
            int cost1 = cost[i-1] + minCosts[i-1];
            // cost to climb current stair using the stair which is 2 steps bellow
            // remember we can take 2 steps at a time
            int cost2 = cost[i-2] + minCosts[i-2];
            // find the min from them
            int currMinCost = Math.min(cost1, cost2);
            // save it in the dp array
            minCosts[i] = currMinCost;
        }
        // return the last index which is the min cost to climb all the stairs and reach at the top floor
        return minCosts[size-1];
    }
