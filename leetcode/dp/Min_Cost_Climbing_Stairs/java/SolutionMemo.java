...
// Time Complexity: O(n) though we are using recursion but due to memoization, each function is getting computed only one time.
// Space Complexity: O(n) for the recursion stack + O(n) for hashmap
public class SolutionMemo {
    ...
    private int getMinCost(int i, int[] cost) {
        // if the subproblem is already solved, let's get the result from the memo
        // we will not recomplute already computed subproblems
        // will compute each subproblems only one time
        // this is why the Time Complexity is O(n) because hashmap lookup time O(1)
        if (this.memo.containsKey(i)) {
            return this.memo.get(i);
        } else {
            // compute first, then cache the result in the memo
            // cost for climbing down 1 stair from the current ith stair
            int cost1 = cost[i-1] + this.getMinCost(i-1, cost);
            // cost for climbing down 2 stairs from the current ith stair
            int cost2 = cost[i-2] + this.getMinCost(i-2, cost);
            this.memo.put(i, Math.min(cost1, cost2));
            return this.memo.get(i);
        }
    }

    public int minCostClimbingStairs(int[] cost) {
        // will do top down recursion, staring from the nth stair
    ...
    public static void main(String[] args) {
        ...
        testCases.stream()
            ...
            .forEach(testCase -> {
                ...
                int actual = new SolutionMemo().minCostClimbingStairs(input);
                if (actual != expected) {
