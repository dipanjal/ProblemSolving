...
class SolutionRecursion:
    def minCostClimbingStairs(self, cost: List[int]) -> int:
        def min_cost(i: int) -> int:
            ...
            # that means when i is 0 or 1 the cost will be zero
            if i == 0 or i == 1:
                return 0
            # computing cost for 1 step down from current stair
            cost_1_step_down = cost[i-1] + min_cost(i-1)
            # computing cost for 2 step down from current stair
            cost_2_step_down = cost[i-2] + min_cost(i-2)
            # taking the minimum cost of the two options
            return min(cost_1_step_down, cost_2_step_down)
        
        # starting from one step above the top of the stairs 
        # and going backwards
        return min_cost(len(cost))
    

# Recursion | Memoization | Top Down | Dynamic Programming
# Time Complexity: O(n) because every problem will be solved once and memoized
# Space Complexity: O(n) because of the memoization + O(n) for the recursion stack
class SolutionMemo:
    def minCostClimbingStairs(self, cost: List[int]) -> int:
        # memoizing base case
        # You can either start from the step with index 0, or the step with index 1.
        ...
        def min_cost(i: int) -> int:
            ...
            if i in memo:
                return memo[i]
            # otherwise, compute the minimum cost for the current stair
            else:
                # computing cost for 1 step down from current stair
                cost_1_step_down = cost[i-1] + min_cost(i-1)
                # computing cost for 2 step down from current stair
                cost_2_step_down = cost[i-2] + min_cost(i-2)
                # taking the minimum cost of the two options
                curr_min_cost = min(cost_1_step_down, cost_2_step_down)
                # memoizing the result
                memo[i] = curr_min_cost
...
def exec_recur(test_suites):
    ...
    for suite in test_suites:
        if suite.get("skip", False):
            continue
        actual = sol.minCostClimbingStairs(suite['input'])
...
if __name__ == "__main__":
    test_suites = [
        {
            "input": [10, 15], 
            "expected": 10 # after paying cost for 0th stair, we will take 2 steps to reach at the top floor
        },
        {
            "input": [10, 15, 20], 
            "expected": 15 # after paying cost for stair at index 1, we will take 2 steps to reach at the top floor
        },
        {
    ...
    # increase the recursion limit if the input array is large
